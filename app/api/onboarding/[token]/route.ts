...
export async function GET(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    const { data: staff } = await client.from('staff_profiles').select('full_name,job_title,employee_number,start_date,location').eq('id', packageRow.staff_id).maybeSingle();
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    const { data: packageRow, error: packageError } = await client.from('staff_onboarding_packages').select('id,staff_id,access_token_expires_at').eq('access_token_hash', hashToken(token)).maybeSingle();
