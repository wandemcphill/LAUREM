...
export async function POST(request: NextRequest) {
  ...
  try {
    const client = db();
    const { data: invite } = await client.from('recruitment_invites').select('id').eq('token_hash', hashToken(token)).maybeSingle();
    ...
    const documentType = String(form.get('documentType') || 'Supporting document').trim().slice(0, 120);
    ...
    const { error: uploadError } = await client.storage.from('laurem-private-documents').upload(path, bytes, { contentType: file.type, upsert: false });
    if (uploadError) throw uploadError;
    const { data, error } = await client.from('recruitment_documents').insert({ application_id: application.id, document_type: documentType, original_filename: file.name.slice(0, 255), storage_path: path, mime_type: file.type, file_size_bytes: file.size, uploaded_by: 'candidate' }).select('id,document_type,original_filename,status,uploaded_at').single();
    if (error) throw error;
    await client.from('recruitment_document_requests').upsert({ application_id: application.id, document_type: documentType, status: 'uploaded' }, { onConflict: 'application_id,document_type' });
