...
export async function GET(request: NextRequest) {
  ...
  let query = db().from('staff_timesheets')
    .select('id,staff_id,assignment_id,work_date,clock_in,clock_out,break_minutes,total_hours,status,notes,approved_by,approved_at,created_at,updated_at,staff_profiles(full_name,email,job_title,employee_number)')
...
export async function PATCH(request: NextRequest) {
  ...
  if (assignmentId) {
    const { data: assignment } = await client.from('staff_assignments').select('id,status').eq('id', assignmentId).eq('staff_id', current.staff_id).maybeSingle();
