...
export async function GET(request: NextRequest, { params }: { params: Promise<{ staffId: string }> }) {
  ...
  const { data: staff, error: staffError } = await client.from('staff_profiles')
    .select('id,application_id,employee_number,full_name,email,phone,job_title,employment_status,start_date,end_date,location,address_line_1,address_line_2,city,county,postcode,country,profile_photo_path,nmc_number,right_to_work_verified,dbs_verified,contract_id,created_at,updated_at')
  ...
  const [assignmentsResult, timesheetsResult, leaveResult, packageResult, auditResult, payrollResult] = await Promise.all([
  ...
  const failed = [assignmentsResult, timesheetsResult, leaveResult, packageResult, auditResult, payrollResult].find((result) => result.error);
  ...
  const { profile_photo_path, ...staffView } = staff;
  const staffResponse = {
    ...staffView,
    profile_photo_url: profile_photo_path
      ? `/api/admin/workforce/staff/${encodeURIComponent(staffId)}/photo`
      : null,
  };
