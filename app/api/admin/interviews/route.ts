...
export async function POST(request: NextRequest) {
  ...
  try{
    const client=db();
    const {data:application,error:appError}=await client.from('laurem_recruitment_applications').select('id,full_name,email,role_applied,living_in_uk').eq('id',applicationId).maybeSingle(); if(appError)throw appError;
    ...
    let {data:attempt,error:attemptError}=await client.from('laurem_interview_attempts').select('id,application_id,round,status').eq('application_id',applicationId).eq('round',1).maybeSingle(); if(attemptError)throw attemptError;
    if(!attempt){
    ...
    const {data:newInvite,error:newInviteError}=await client.from('laurem_recruitment_invites').insert({candidate_name:application.full_name,candidate_email:application.email,role,token_hash:hashToken(assessmentToken),expires_at:assessmentExpiresAt,used_at:null}).select('id').single();
    if(newInviteError||!newInvite)throw new Error(newInviteError?.message||'Unable to create assessment invitation.');
    const {error:rebindError}=await client.from('laurem_recruitment_applications').update({invite_id:newInvite.id,updated_at:new Date().toISOString()}).eq('id',applicationId); if(rebindError)throw rebindError;
    const {error:attemptInviteError}=await client.from('laurem_interview_attempts').update({invite_id:newInvite.id,updated_at:new Date().toISOString()}).eq('id',attempt.id); if(attemptInviteError)throw attemptInviteError;
    ...
    return withRequestId(NextResponse.json({interview:data,assessment:{link:assessmentLink,expiresAt:assessmentExpiresAt,questions:ROUND1_QUESTIONS_PER_ATTEMPT},email:{status:email.status,attempts:email.attempts,providerId:'providerId' in email?email.providerId:null,deliveryId:email.deliveryId,...(email.status==='failed'?{error:email.error}:{} )}},{status:201}), requestId);
  }catch(error){logOperationalError({requestId,event:'admin.interview.schedule_failed',actor:session.email,reason:error});return operationalError(requestId,'Unable to schedule interview.',500,'INTERVIEW_SCHEDULE_FAILED');}
}
...
export async function PATCH(request: NextRequest) {
  ...
  const raw=await request.text(); if(new TextEncoder().encode(raw).byteLength>32000)return NextResponse.json({error:'Request payload is too large.'},{status:413});
  let body:Record<string,unknown>; try{body=JSON.parse(raw) as Record<string,unknown>;}catch{return NextResponse.json({error:'Invalid JSON.'},{status:400});}
  const client=db(); const {data:current,error:currentError}=await client.from('laurem_recruitment_interviews').select('*,application:recruitment_applications(id,full_name,email)').eq('id',id).maybeSingle();
  ...
  const {data,error}=await client.from('laurem_recruitment_interviews').update(patch).eq('id',id).select('*').maybeSingle(); if(error)return NextResponse.json({error:'Unable to update interview.'},{status:500}); if(!data)return NextResponse.json({error:'Interview not found.'},{status:404});
