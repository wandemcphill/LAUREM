...
export async function GET(req: NextRequest) {
  ...
  const { data: conversations } = await client.from('staff_message_conversations')
    .select('id,updated_at,last_message_at').in('id', ids).order('last_message_at', { ascending: false, nullsFirst: false });
  const out: any[] = [];
  for (const conversation of conversations || []) {
    const { data: participants } = await client.from('staff_message_participants')
      .select('staff_id,last_read_at').eq('conversation_id', conversation.id);
...
export async function POST(req: NextRequest) {
  ...
  const { data: created, error } = await client.from('staff_messages').insert({ conversation_id: conversation.id, sender_staff_id: session.staff_id, body: message })
