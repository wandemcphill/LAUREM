...
export async function GET(req: NextRequest, context: Context) {
  ...
  const { data: other } = otherId ? await client.from('laurem_staff_profiles').select('id,full_name,laurem_id,employee_number,job_title').eq('id', otherId).maybeSingle() : { data: null };
...
export async function POST(req: NextRequest, context: Context) {
  ...
  const ids = await participantConversationIds(client, session.staff_id);
  if (!ids.includes(id)) return NextResponse.json({ error: 'Conversation not found.' }, { status: 404 });
  const { data: created, error } = await client.from('laurem_staff_messages').insert({ conversation_id: id, sender_staff_id: session.staff_id, body: message })
