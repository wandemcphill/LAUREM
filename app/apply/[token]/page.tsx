...
const emptyEmploymentEntry = (): EmploymentEntry => ({ employer: '', jobTitle: '', from: '', to: '', responsibilities: '', reasonForLeaving: '' });
...
export default function ApplicationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const selectedRole = draft.role_applied as string;
  ...
  function updateNested(group: string, key: string, value: unknown) {
    setDraft((current) => ({ ...current, [group]: { ...(current[group] as Record<string, unknown> || {}), [key]: value } }));
    setError(null);
  }
  ...
  const experienceStepTitle = isNurse ? 'Nursing & experience' : selectedRole === 'Physiotherapist' ? 'Professional registration & experience' : 'Qualifications & experience';
  ...
  <form onSubmit={submit}>
    {step === 0 && <Card title="Role and pathway">
      <label>Role<select required value={String(draft.role_applied)} onChange={(e) => update('role_applied', e.target.value)}>
      ...
      <label>Application pathway<div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>{['uk', 'international'].map((p) => <button type="button" key={p} onClick={() => update('pathway', p)} style={{ padding: '11px 14px', borderRadius: 9, border: '1px solid var(--line)', background: draft.pathway === p ? 'var(--ink)' : 'white', color: draft.pathway === p ? 'white' : 'var(--ink)', fontWeight: 700 }}>{p === 'uk' ? 'UK-based applicant' : 'International applicant'}</button>)}</div></label>
      ...
      {draft.pathway === 'international' && !isNurse && <Notice>International applications for care and support roles are subject to current UK immigration eligibility and any applicable in-country route requirements.</Notice>}
    </Card>}
    ...
    {step === 2 && <Card title={experienceStepTitle}>
      {isNurse ? <>
        <Grid><Input label="NMC number" value={String(draft.nmc_number)} onChange={(v) => update('nmc_number', v)} required /><Input label="RCN number" value={String(draft.rcn_number)} onChange={(v) => update('rcn_number', v)} /><Input label="Band / grade" value={String(draft.band)} onChange={(v) => update('band', v)} /></Grid>
        <TextArea label="Qualifications" value={String(draft.qualifications)} onChange={(v) => update('qualifications', v)} required />
        <TextArea label="Nursing / care experience" value={String(draft.care_experience)} onChange={(v) => update('care_experience', v)} required />
        <TextArea label="Professional experience and registrations" value={String(draft.professional_experience)} onChange={(v) => update('professional_experience', v)} required />
      </> : <>
        <Grid>{selectedRole === 'Physiotherapist' && <Input label="Professional registration / regulatory details" value={String(draft.professional_registration)} onChange={(v) => update('professional_registration', v)} />}</Grid>
        <TextArea label="Qualifications" value={String(draft.qualifications)} onChange={(v) => update('qualifications', v)} required />
        <TextArea label={selectedRole === 'Physiotherapist' ? 'Physiotherapy experience' : 'Care / support experience'} value={String(draft.care_experience)} onChange={(v) => update('care_experience', v)} required />
        <TextArea label="Professional experience and relevant skills" value={String(draft.professional_experience)} onChange={(v) => update('professional_experience', v)} required />
      </>}
    ...
    {step === 3 && <Card title="References and employment history"><h3>Professional references</h3>{draft.references.map((ref, i) => <div key={i} style={{ border: '1px solid var(--line)', borderRadius: 12, padding: 14, marginBottom: 12 }}><strong>Reference {i + 1}</strong><Grid>{['name','position','organisation','email','telephone'].map((key) => <Input key={key} label={key[0].toUpperCase() + key.slice(1)} value={ref[key] || ''} onChange={(v) => setDraft((d) => ({ ...d, references: d.references.map((r, idx) => idx === i ? { ...r, [key]: v } : r) }))} required={i < 2 && ['name','organisation','email','telephone'].includes(key)} />)}</Grid></div>)}<h3>Employment history</h3>{draft.employment_history.map((entry, i) => <div key={i} style={{ border: '1px solid var(--line)', borderRadius: 12, padding: 14, marginBottom: 12 }}><strong>Employment {i + 1}</strong><Grid><Input label="Employer" value={entry.employer} onChange={(v) => updateEmployment(i, 'employer', v)} /><Input label="Job title" value={entry.jobTitle} onChange={(v) => updateEmployment(i, 'jobTitle', v)} /><Input label="From" type="date" value={entry.from} onChange={(v) => updateEmployment(i, 'from', v)} /><Input label="To" type="date" value={entry.to} onChange={(v) => updateEmployment(i, 'to', v)} /></Grid><TextArea label="Responsibilities" value={entry.responsibilities} onChange={(v) => updateEmployment(i, 'responsibilities', v)} /><Input label="Reason for leaving" value={entry.reasonForLeaving} onChange={(v) => updateEmployment(i, 'reasonForLeaving', v)} /><button type="button" onClick={() => removeEmployment(i)} style={{ ...buttonSecondary, marginTop: 10 }}>Remove employment</button></div>)}<button type="button" onClick={addEmployment} style={buttonSecondary}>Add another employment</button></Card>}
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginTop: 16 }}>{step > 0 ? <button type="button" onClick={() => setStep(step - 1)} style={buttonSecondary}>Back</button> : <span />}{step < steps.length - 1 ? <button type="button" onClick={() => setStep(step + 1)} disabled={step === 0 && !selectedRole} style={buttonPrimary}>Continue</button> : <button type="submit" disabled={saving} style={buttonPrimary}>{saving ? 'Submitting...' : 'Submit application'}</button>}</div>
  </form>
