...
export default function ActivationForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Unable to activate portal.');
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#f4f7fb', fontFamily: 'system-ui', padding: 20 }}>
      <form onSubmit={submit} style={{ background: '#fff', padding: 28, border: '1px solid #e5eaf0', borderRadius: 18, width: 'min(440px,100%)' }}>
        ...
        <button type="submit" disabled={busy || !token} style={{ width: '100%', marginTop: 12, padding: 12, border: 0, borderRadius: 10, background: '#0f766e', color: '#fff', fontWeight: 900 }}>{busy ? 'Activating…' : 'Activate portal'}</button>
