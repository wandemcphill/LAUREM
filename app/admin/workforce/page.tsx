...
export default function WorkforcePage() {
  ...
  async function load() {
    ...
    try {
      ...
      setStaff(sp.staff||[]); setAssignments(ap.assignments||[]); setTimesheets(tp.timesheets||[]); setLeave(lp.leaveRequests||[]);
  ...
  async function updateStaff(id:string, employmentStatus:string, endDate?:string) {
    setBusy(true); setError('');
    try { const r=await fetch(`/api/admin/workforce/staff?id=${encodeURIComponent(id)}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({employmentStatus,endDate})}); const p=await r.json(); if(!r.ok) throw new Error(p.error||'Unable to update staff.'); await load(); }
  ...
  return <main className="wrap" style={{padding:'36px 0 80px'}}>
    ...
    <section style={{marginTop:28}}><h2>Staff</h2><div style={{display:'grid',gap:10}}>{staff.map(s=><article className="card" key={s.id} style={{padding:16,display:'flex',justifyContent:'space-between',gap:14,flexWrap:'wrap',alignItems:'center'}}><div><strong>{s.full_name}</strong><div style={{color:'var(--muted)',fontSize:13}}>{s.employee_number} · {s.job_title} · {s.location||'Location not set'}</div></div><div style={{display:'flex',gap:8,alignItems:'center'}}><span style={{padding:'6px 9px',borderRadius:999,background:'var(--soft)',fontSize:12,fontWeight:800}}>{s.employment_status}</span>{s.employment_status==='pending'&&<button disabled={busy} onClick={()=>void updateStaff(s.id,'active')} style={{padding:'7px 10px'}}>Activate</button>}{s.employment_status==='active'&&<><button disabled={busy} onClick={()=>void updateStaff(s.id,'suspended')} style={{padding:'7px 10px'}}>Suspend</button><button disabled={busy} onClick={()=>void updateStaff(s.id,'leaver',new Date().toISOString().slice(0,10))} style={{padding:'7px 10px'}}>Mark leaver</button></>}{s.employment_status==='suspended'&&<button disabled={busy} onClick={()=>void updateStaff(s.id,'active')} style={{padding:'7px 10px'}}>Reactivate</button>}</div></article>)}</div></section>
