...
type Staff = { id:string; application_id:string|null; employee_number:string; full_name:string; email:string; phone:string|null; job_title:string; employment_status:string; start_date:string|null; end_date:string|null; location:string|null; nmc_number:string|null; right_to_work_verified:boolean; dbs_verified:boolean; contract_id:string|null; address_line_1:string|null; address_line_2:string|null; city:string|null; county:string|null; postcode:string|null; country:string|null; profile_photo_path:string|null; profile_photo_updated_at:string|null; profile_photo_url?:string|null; created_at:string; updated_at:string };
...
export default function Staff360Page({ params }: { params:Promise<{ staffId:string }> }) {
  ...
  async function load() {
    ...
    try {
      ...
      setData(body); setPhotoFailed(false);
    ...
    <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'center',flexWrap:'wrap'}}>
      <div style={{display:'flex',gap:14,alignItems:'center',minWidth:0}}>
        <div style={{width:76,height:76,borderRadius:'50%',overflow:'hidden',background:'var(--soft)',display:'grid',placeItems:'center',flexShrink:0}}>
          {s.profile_photo_url&&!photoFailed ? <img src={s.profile_photo_url} alt='LAUREM staff photograph' onError={()=>setPhotoFailed(true)} style={{width:'100%',height:'100%',objectFit:'cover'}}/> : <span style={{fontWeight:900,color:'var(--ink)',fontSize:24}}>{s.full_name.split(' ').map((part)=>part[0]).filter(Boolean).slice(0,2).join('').toUpperCase()}</span>}
        </div>
...
function Overview({staff,data,stats}:{staff:Staff;data:StaffPayload;stats:{upcoming:number;pendingTimesheets:number;pendingLeave:number;requiredDone:number;requiredTotal:number;openPayroll:number}}) { return <section style={{display:'grid',gridTemplateColumns:'minmax(0,1.5fr) minmax(300px,1fr)',gap:16,marginTop:20}}><article className="card" style={{padding:20}}><h2 style={{marginTop:0}}>Employment profile</h2><dl style={dl}><Row label="Employee number" value={staff.employee_number}/><Row label="Role" value={staff.job_title}/><Row label="Email" value={staff.email}/><Row label="Phone" value={staff.phone||'Not set'}/><Row label="Location" value={staff.location||'Not set'}/><Row label="Start date" value={dateOnly(staff.start_date)}/><Row label="End date" value={dateOnly(staff.end_date)}/><Row label="Address" value={[staff.address_line_1,staff.address_line_2,staff.city,staff.county,staff.postcode,staff.country].filter(Boolean).join(', ')||'Not set'}/><Row label="NMC number" value={staff.nmc_number||'Not applicable / not recorded'}/></dl></article><article className="card" style={{padding:20}}><h2 style={{marginTop:0}}>Availability & preferences</h2>{data.availability?<><div style={{fontWeight:900}}>{[data.availability.full_time?'Full-time':data.availability.part_time?'Part-time':null,data.availability.days?'Day shifts':null,data.availability.nights?'Night shifts':null,data.availability.weekends?'Weekends':null].filter(Boolean).join(' · ')}</div><div style={muted}>Effective {dateOnly(data.availability.effective_from)}</div>{data.availability.notes&&<p style={{color:'var(--muted)',whiteSpace:'pre-wrap'}}>{data.availability.notes}</p>}</>:<div style={muted}>No staff availability has been submitted.</div>}</article><article className="card" style={{padding:20}}><h2 style={{marginTop:0}}>Compliance snapshot</h2><div style={compliance}><Check label="Right to work verified" value={staff.right_to_work_verified}/><Check label="DBS verified" value={staff.dbs_verified}/><Check label="Accepted contract linked" value={Boolean(staff.contract_id)}/><Check label="Application linked" value={Boolean(staff.application_id)}/><Check label="Employment documents issued" value={data.documentsSummary.total>0}/></div><div style={{marginTop:18,paddingTop:16,borderTop:'1px solid var(--line)',color:'var(--muted)',fontSize:13}}>{stats.requiredDone}/{stats.requiredTotal} required onboarding tasks complete · {stats.openPayroll} open payroll entries</div><div style={{marginTop:8,color:'var(--muted)',fontSize:12}}>{data.documentsSummary.total} issued document{data.documentsSummary.total===1?'':'s'} · {data.documentsSummary.signaturePending} awaiting signature</div></article></section>; }
