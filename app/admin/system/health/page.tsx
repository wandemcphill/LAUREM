...
export default function SystemHealthPage() {
  ...
  return <main className="wrap" style={{ padding: '34px 0 80px', maxWidth: 1080 }}>
    ...
    <header style={{ margin: '18px 0 24px' }}>
      ...
      <p style={{ color: 'var(--muted)', maxWidth: 760 }}>Dependency and deployment checks for the private LAUREM Portal. This page intentionally reports health signals, not secrets or candidate records.</p>
    </header>
    ...
    {health && <>
      <section className="card" style={{ padding: 22, marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <div><div style={{ color: 'var(--muted)', fontSize: 12, fontWeight: 800, letterSpacing: '.08em' }}>OVERALL</div><h2 style={{ margin: '6px 0' }}>{health.ok ? 'System ready' : 'System not ready'}</h2><p style={{ color: 'var(--muted)', margin: 0 }}>Checked {new Date(health.timestamp).toLocaleString()}</p></div>
          <Badge ok={health.ok} />
        </div>
      </section>
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(235px,1fr))', gap: 14 }}>
        ...
        <Check title="Schema" ok={health.dependencies.schema.ok} detail={health.dependencies.schema.ok ? 'Expected portal tables are present.' : `Missing: ${health.dependencies.schema.missing.join(', ')}`} />
