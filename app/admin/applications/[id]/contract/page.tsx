...
export default function InternationalNurseContractPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/applications', { cache: 'no-store' })
      ...
      .then((p) => {
        const found = (p.applications || []).find((a: Application) => a.id === id) || null;
        setApplication(found);
        if (found && !(found.role_applied === 'Registered Nurse - International Recruitment' || (found.role_applied === 'Registered Nurse' && found.living_in_uk === 'No'))) {
          router.replace(`/admin/applications/${encodeURIComponent(id)}/contract/standard`);
        }
      })
  ...
  if (!eligible) return <main className="wrap" style={{ padding: 40 }}><h1>Redirecting</h1><p>This candidate uses the standard employment contract workflow.</p><Link href={`/admin/applications/${id}/contract/standard`}>Open standard contract</Link></main>;
