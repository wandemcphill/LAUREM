...
type Application = Record<string, unknown> & { id: string; full_name: string; email: string; role_applied: string; status: string; application_data?: Record<string, unknown> };
...
export default function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="wrap" style={{ padding: '34px 0 80px', maxWidth: 1080 }}><Link href="/admin" style={{ color: 'var(--muted)', textDecoration: 'none' }}>← Recruiter workspace</Link><header style={{ margin: '18px 0 24px' }}><p style={{ color: 'var(--accent)', fontWeight: 800, letterSpacing: '.08em' }}>{lauremCompany.tradingName.toUpperCase()} CANDIDATE FILE</p><h1 style={{ marginBottom: 5 }}>{application.full_name}</h1><p style={{ color: 'var(--muted)' }}>{application.role_applied} · {application.email}</p></header>{notice && <div role="alert" className="card" style={{ padding: 14, marginBottom: 14 }}>{notice}</div>}

<section className="card" style={{ padding: 22, marginBottom: 14 }}><h2>Recruitment status</h2><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><select value={status} disabled={saving} onChange={(e) => changeStatus(e.target.value)} style={{ padding: 11, border: '1px solid var(--line)', borderRadius: 9 }}>{recruitmentConfig.statusFlow.map((s) => <option key={s}>{s}</option>)}</select><button disabled={saving} onClick={() => action('/api/admin/second-interviews', { applicationId: id })} style={buttonPrimary}>Create second-interview link</button><button disabled={saving} onClick={() => action('/api/admin/contracts', { applicationId: id, minimumWeeklyHours: 40 })} style={buttonPrimary}>Generate contract</button><button disabled={saving} onClick={() => action('/api/admin/onboarding', { applicationId: id })} style={buttonSecondary}>Convert to staff</button></div></section>

<section className="card" style={{ padding: 22, marginBottom: 14 }}><h2>Application information</h2><div style={grid}><Info label="Phone" value={application.phone} /><Info label="Nationality" value={application.nationality} /><Info label="Country" value={application.country_of_residence} /><Info label="Pathway" value={application.living_in_uk === 'No' ? 'International' : 'UK'} /><Info label="Start date" value={application.start_date} /><Info label="Work permission" value={application.work_permission} /></div><Detail title="Qualifications" value={application.qualifications} /><Detail title="Care / professional experience" value={application.care_experience || application.professional_experience} /><Detail title="Application payload" value={JSON.stringify(application.application_data || {}, null, 2)} /></section>
...
<p style={{ color: 'var(--muted)', fontSize: 12, marginTop: 18 }}>Contract wording is generated from the approved Laurem template and the supplied source structure. Final employment terms should be legally reviewed before production issue.</p></main>;
}
...
const buttonPrimary = { background: 'var(--ink)', color: 'white', border: 0, padding: '11px 14px', borderRadius: 9, fontWeight: 800 };
