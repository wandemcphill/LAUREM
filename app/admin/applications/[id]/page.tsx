...
export default function Candidate360Page(){
  ...
  async function sendSecondInterview(){
    ...
    try{
      ...
      setNotice(body.email?.status==='sent'?'Second interview link emailed to the candidate.':'Second interview link created, but email delivery needs attention.');
      await load();
    }catch(e){setError(e instanceof Error?e.message:'Unable to create second interview.');}
    finally{setBusy(false);}
  }
  ...
  async function reviewDocument(documentId:string,status:'approved'|'rejected'){
    ...
    try{
      ...
      if(status==='rejected'&&!reviewNote){setBusy(false);return;}
      const response=await fetch(`/api/admin/documents?id=${encodeURIComponent(documentId)}`,{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({status,reviewNote})});
      const body=await response.json();
      if(!response.ok)throw new Error(body.error||'Unable to review document.');
      setNotice(`Document ${status}.`);await load();
  ...
  return <main style={{minHeight:'100vh',background:'#f4f7fb',color:'#102a43',fontFamily:'system-ui',padding:'26px 18px 70px'}}>
    <div style={{maxWidth:1160,margin:'0 auto'}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'flex-start',flexWrap:'wrap'}}>
        <div><Link href="/admin" style={buttonBase}>← Recruiter workspace</Link><div style={{marginTop:18,fontSize:12,fontWeight:900,letterSpacing:1.3,color:'#0f766e'}}>LAUREM CANDIDATE 360</div><h1 style={{fontSize:40,margin:'5px 0 5px'}}>{a.full_name}</h1><div style={muted}>{a.role_applied||'Role not set'} · {a.email}</div></div>
      ...
      <section style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:10,marginTop:18}}><Metric label="Readiness blockers" value={requiredOpen}/><Metric label="Approved documents" value={approvedDocs}/><Metric label="Second interviews" value={data.secondInterviews.length}/><Metric label="Contracts" value={data.contracts.length}/></section>

      <section style={{...card,marginTop:18}}><div style={row}><div><h2 style={{margin:'0 0 5px'}}>Recruitment controls</h2><div style={muted}>Every lifecycle change is recorded against this candidate.</div></div><select value={a.status} disabled={busy} onChange={e=>void changeStatus(e.target.value)} style={{padding:11,border:'1px solid #d9e2ec',borderRadius:9,fontWeight:700}}>{recruitmentConfig.statusFlow.map((status:string)=><option key={status}>{status}</option>)}</select></div><div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:14}}><button disabled={busy} onClick={()=>void sendSecondInterview()} style={primary}>Send second interview</button><Link href={`/admin/applications/${encodeURIComponent(id)}/contract`} style={buttonBase}>Prepare contract</Link><Link href={`/admin/applications/${encodeURIComponent(id)}/readiness`} style={buttonBase}>Readiness gate</Link></div></section>

      <section style={{display:'grid',gridTemplateColumns:'minmax(0,1.3fr) minmax(290px,.7fr)',gap:16,marginTop:16}}><article style={card}><h2 style={{marginTop:0}}>Candidate identity</h2><div style={grid}><Info label="Full name" value={a.full_name}/><Info label="Preferred name" value={a.preferred_name}/><Info label="Email" value={a.email}/><Info label="Phone" value={a.phone}/><Info label="Date of birth" value={a.date_of_birth}/><Info label="Nationality" value={a.nationality}/><Info label="Country of residence" value={a.country_of_residence}/><Info label="Address" value={a.address}/><Info label="Employment type" value={a.employment_type}/><Info label="Start date" value={a.start_date}/><Info label="Consent" value={a.consent?'Yes':'No'}/></div></article><article style={card}><h2 style={{marginTop:0}}>Invitation</h2><Info label="Candidate" value={data.invite?.candidate_name}/><Info label="Invitation role" value={data.invite?.role}/><Info label="Created" value={fmt(data.invite?.created_at)}/><Info label="Expires" value={fmt(data.invite?.expires_at)}/><Info label="Used" value={fmt(data.invite?.used_at)}/><p style={{...muted,fontSize:12,marginTop:18}}>The private token itself is never displayed here.</p></article></section>
