...
<section className="card" style={{ padding: 22, marginBottom: 14 }}><h2>Recruitment status</h2><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><select value={status} disabled={saving} onChange={(e) => changeStatus(e.target.value)} style={{ padding: 11, border: '1px solid var(--line)', borderRadius: 9 }}>{recruitmentConfig.statusFlow.map((s) => <option key={s}>{s}</option>)}</select><button disabled={saving} onClick={() => action('/api/admin/second-interviews', { applicationId: id })} style={buttonPrimary}>Create second-interview link</button>{internationalNurse ? <Link href={`/admin/applications/${id}/contract`} style={buttonPrimary}>Prepare international nurse contract</Link> : <button disabled={saving} onClick={() => action('/api/admin/contracts', { applicationId: id, minimumWeeklyHours: 40 })} style={buttonPrimary}>Generate contract</button>}<button disabled={saving} onClick={() => action('/api/admin/onboarding', { applicationId: id, rightToWorkVerified: true, dbsVerified: true })} style={buttonSecondary}>Convert to staff</button></div></section>
...
<p style={{ color: 'var(--muted)', fontSize: 12, marginTop: 18 }}>International nurse onboarding includes the Laurem overseas nurse handbook and UK/Scotland welcome package. Staff conversion requires an accepted contract and records onboarding tasks against the staff profile.</p></main>;
}
...
const buttonSecondary = { background: 'white', color: 'var(--ink)', border: '1px solid var(--line)', padding: '11px 14px', borderRadius: 9, fontWeight: 800, cursor: 'pointer' };