...
export default function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [onboardingAdminLink, setOnboardingAdminLink] = useState<string | null>(null);
  ...
  async function action(url: string, body: Record<string, unknown>) {
    setSaving(true); setNotice(null); setResultLink(null); setOnboardingAdminLink(null);
    try {
      const response = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) });
      const payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Action failed.');
      if (payload.acceptanceLink) setResultLink(payload.acceptanceLink);
      else if (payload.link) setResultLink(payload.link);
      if (payload.onboardingLink) setResultLink(payload.onboardingLink);
      if (payload.staff?.id) setOnboardingAdminLink(`/admin/onboarding/${encodeURIComponent(payload.staff.id)}`);
  ...
  return <main className="wrap" style={{ padding: '34px 0 80px', maxWidth: 1080 }}><Link href="/admin" style={{ color: 'var(--muted)', textDecoration: 'none' }}>← Recruiter workspace</Link><header style={{ margin: '18px 0 24px' }}><p style={{ color: 'var(--accent)', fontWeight: 800, letterSpacing: '.08em' }}>{lauremCompany.tradingName.toUpperCase()} CANDIDATE FILE</p><h1>{application.full_name}</h1><p style={{ color: 'var(--muted)' }}>{application.role_applied} · {application.email}</p></header>{notice && <div role="alert" className="card" style={{ padding: 14, marginBottom: 14 }}>{notice}</div>}{resultLink && <div className="card" style={{ padding: 14, marginBottom: 14 }}>Candidate link: <a href={resultLink} target="_blank" rel="noreferrer">{resultLink}</a></div>}{onboardingAdminLink && <div className="card" style={{ padding: 14, marginBottom: 14 }}><Link href={onboardingAdminLink}>Open staff onboarding workspace →</Link></div>}

<section className="card" style={{ padding: 22, marginBottom: 14 }}><h2>Recruitment status</h2><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><select value={status} disabled={saving} onChange={(e) => changeStatus(e.target.value)} style={{ padding: 11, border: '1px solid var(--line)', borderRadius: 9 }}>{recruitmentConfig.statusFlow.map((s) => <option key={s}>{s}</option>)}</select><button disabled={saving} onClick={() => action('/api/admin/second-interviews', { applicationId: id })} style={buttonPrimary}>Create second-interview link</button>{internationalNurse ? <Link href={`/admin/applications/${id}/contract`} style={buttonPrimary}>Prepare international nurse contract</Link> : <button disabled={saving} onClick={() => action('/api/admin/contracts', { applicationId: id, minimumWeeklyHours: 40 })} style={buttonPrimary}>Generate contract</button>}<Link href={`/admin/applications/${id}/readiness`} style={buttonSecondary}>Open readiness gate</Link><button disabled={saving} onClick={() => action('/api/admin/onboarding', { applicationId: id, dbsVerified: true })} style={buttonSecondary}>Convert to staff</button></div></section>
...
<p style={{ color: 'var(--muted)', fontSize: 12, marginTop: 18 }}>Staff conversion now requires an accepted contract with a matching canonical role and a completed pre-staff readiness gate. International nurse onboarding also retains the Laurem overseas nurse handbook and UK/Scotland welcome package.</p></main>;
}
...
const buttonPrimary = { background: 'var(--ink)', color: 'white', border: 0, padding: '11px 14px', borderRadius: 9, fontWeight: 800, textDecoration: 'none', display: 'inline-block', cursor: 'pointer' };
const buttonSecondary = { background: 'white', color: 'var(--ink)', border: '1px solid var(--line)', padding: '11px 14px', borderRadius: 9, fontWeight: 800, textDecoration: 'none', display: 'inline-block', cursor: 'pointer' };