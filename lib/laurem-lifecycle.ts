...
export class LauremLifecycleError extends Error {
  constructor(
    public readonly code: LauremLifecycleErrorCode,
    message: string,
...
export async function loadAcceptedLauremContract(client: SupabaseClient, applicationId: string): Promise<ContractRecord> {
  const { data, error } = await client
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data || data.status !== 'accepted' || !data.accepted_at) {
    throw new LauremLifecycleError(
      'CONTRACT_REQUIRED',
      'The employment contract must be accepted before this lifecycle transition can proceed.',
    );
  }
...
export function assertLauremContractRole(application: ApplicationRecord, contract: ContractRecord) {
  ...
  if (!applicationRole || !contractRole || applicationRole !== contractRole) {
    throw new LauremLifecycleError(
      'CONTRACT_ROLE_MISMATCH',
      "The accepted contract role does not match the candidate's applied role.",
      { applicationRole, contractRole },
    );
  }
}
...
export async function assertLauremOnboardingReady(client: SupabaseClient, application: ApplicationRecord) {
  ...
  if (!readiness.ready) {
    throw new LauremLifecycleError(
      'READINESS_INCOMPLETE',
      `Onboarding readiness is incomplete. Complete the following before this lifecycle transition: ${readiness.missing.map((item) => item.title).join(', ')}`,
      { readiness },
    );
  }
  return readiness;
...
export function assertLauremStaffContractBinding(staff: StaffRecord, acceptedContract: ContractRecord) {
  if (staff.contract_id && staff.contract_id !== acceptedContract.id) {
    throw new LauremLifecycleError(
      'STAFF_CONTRACT_MISMATCH',
      'The existing staff profile is linked to a different employment contract.',
      { staffContractId: staff.contract_id, acceptedContractId: acceptedContract.id },
    );
  }
}
...
export async function validateLauremStaffTransition(client: SupabaseClient, applicationId: string) {
  ...
  const contract = await loadAcceptedLauremContract(client, applicationId);
  assertLauremContractRole(application, contract);
  ...
  const { data: staff, error } = await client
    ...
    .maybeSingle();
  if (error) throw error;
  if (staff) assertLauremStaffContractBinding(staff as StaffRecord, contract);
