-- Forward-only correction for the staff onboarding RPC.
-- LAUREM recruitment applications store optional NMC data in application_data JSONB;
-- the composite row type has no nmc_number column. Keep the gated onboarding boundary
-- unchanged while reading the optional value from the canonical JSONB field.
