...
begin
  ...
  from unnest(expected_columns) e
  where not exists (
    select 1
    from information_schema.columns c
    where c.table_schema = 'public'
      and c.table_name = e[1]
      and c.column_name = e[2]
  );
  ...
  select exists (
    ...
    where m.version = '20260920184540'
  )
