...
declare
  expected_tables text[] := array[
    ...
    'laurem_data_governance_policies','laurem_data_governance_findings','laurem_data_governance_events','laurem_data_access_requests'
  ];
  expected_columns jsonb := '[
    ...
    ["laurem_data_governance_policies","id"],["laurem_data_governance_policies","rule_key"],["laurem_data_governance_policies","owner_role"],["laurem_data_governance_policies","action"],
    ["laurem_data_governance_findings","id"],["laurem_data_governance_findings","finding_key"],["laurem_data_governance_findings","finding_type"],["laurem_data_governance_findings","status"],
    ["laurem_data_access_requests","id"],["laurem_data_access_requests","request_type"],["laurem_data_access_requests","subject_type"],["laurem_data_access_requests","status"]
  ]'::jsonb;
  expected_indexes text[] := array[
    ...
    'laurem_data_findings_status_severity_idx','laurem_data_findings_subject_idx','laurem_data_findings_application_idx','laurem_data_findings_staff_idx','laurem_data_events_finding_idx','laurem_data_requests_status_idx','laurem_data_requests_subject_idx'
  ];
  missing_tables text[];
...
begin
  select coalesce(array_agg(e.table_name order by e.table_name), array[]::text[]) into missing_tables
  from unnest(expected_tables) e(table_name)
  where not exists (select 1 from information_schema.tables t where t.table_schema='public' and t.table_name=e.table_name and t.table_type='BASE TABLE');

  select coalesce(array_agg(e.table_name order by e.table_name), array[]::text[]) into rls_disabled_tables
  from unnest(expected_tables) e(table_name)
  join pg_class c on c.relname=e.table_name
  join pg_namespace n on n.oid=c.relnamespace and n.nspname='public'
  where coalesce(c.relrowsecurity,false)=false;

  select coalesce(jsonb_agg(jsonb_build_object('table',x.value->>0,'column',x.value->>1) order by x.value->>0,x.value->>1),'[]'::jsonb) into missing_columns
  from jsonb_array_elements(expected_columns) x
  where not exists (select 1 from information_schema.columns c where c.table_schema='public' and c.table_name=x.value->>0 and c.column_name=x.value->>1);

  select coalesce(array_agg(e.index_name order by e.index_name),array[]::text[]) into missing_indexes
  from unnest(expected_indexes) e(index_name)
  where not exists (select 1 from pg_indexes p where p.schemaname='public' and p.indexname=e.index_name);

  select exists(select 1 from supabase_migrations.schema_migrations m where m.version='20260920184540') into baseline_migration_present;
  select jsonb_build_object('version',m.version,'name',m.name) into latest_migration from supabase_migrations.schema_migrations m order by m.version desc limit 1;

  return jsonb_build_object(
    'ok',cardinality(missing_tables)=0 and cardinality(rls_disabled_tables)=0 and jsonb_array_length(missing_columns)=0 and cardinality(missing_indexes)=0 and baseline_migration_present,
    'contract_version',3,
    'expected_table_count',cardinality(expected_tables),
    'missing_tables',missing_tables,'rls_disabled_tables',rls_disabled_tables,'missing_columns',missing_columns,'missing_indexes',missing_indexes,
    'baseline_migration_present',baseline_migration_present,'latest_migration',latest_migration,'checked_at',clock_timestamp()
  );
end;
$$;
